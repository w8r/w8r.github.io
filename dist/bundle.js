(function () {
  ...
  function longitude(point) {
    if (abs(point[0]) <= pi$3)
      ...
      return sign(point[0]) * ((abs(point[0]) + pi$3) % tau$1 - pi$3);
  }
  ...
  const regl$1 = regl(document.body);
  ...
  loadJson('https://unpkg.com/world-atlas@1/world/110m.json').then(world => {
    ...
    const boundary = mesh(world, world.objects.countries, (a, b) => a !== b);

    const usMesh = mesh(world);
    ...
    const elements = regl$1.elements({
      primitive: 'lines',
      data: indexes
    });
